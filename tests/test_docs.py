...
class DemoReportFreshnessTest(unittest.TestCase):
    ...
    The artifact is generated by the full (AST) engine; the regex-fallback
    matrix entry legitimately produces different findings for the demo
    bundle, so the byte-exact freshness check only runs with an AST engine
    active (CI's no-ast-parser job must still pass — it tests degradation,
    not payload freshness).
    """

    def test_demo_report_is_up_to_date(self):
        from core.js_parser import parser_status
        if parser_status()["mode"] != "ast":
            self.skipTest("demo payload is a full-engine artifact; "
                          "not regenerable under the regex fallback")
        # The committed artifact records the engine that produced it; only
        # that engine can verify it byte-exactly (different AST engines can
        # report the demo bundle slightly differently).
        import json
        with open(os.path.join(ROOT, "webui", "demo", "report.js"),
                  encoding="utf-8") as fh:
            text = fh.read()
        artifact = json.loads(
            text.split("window.SS_DEMO_REPORT = ", 1)[1].rsplit(";", 1)[0])
        demo_engine = artifact.get("meta", {}).get("demo_engine")
        if demo_engine and demo_engine != parser_status()["name"]:
            self.skipTest(f"demo payload was generated by the {demo_engine} "
                          f"engine; this environment runs "
                          f"{parser_status()['name']}")
